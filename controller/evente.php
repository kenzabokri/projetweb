...
class eventc
{
    ...
    {
        $sql = "DELETE FROM eve WHERE idevent = :idevent";
        $db = config::getConnexion();
        $req = $db->prepare($sql);
    ...
    function add($event)
{
    $sql = "INSERT INTO eve (nomevent, description, date, heuredebut, heurefin, capacite, image, lieu, idtype)  
    ...
    {
        $sql = "SELECT * FROM eve WHERE idevent = :idevent";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
